...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: renos.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: renosd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeRenosCoinamount=<n> " + _("Keep N RenosCoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. RenosCoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. RenosCoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("RenosCoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    nAnonymizeRenosCoinAmount = GetArg("-anonymizeRenosCoinamount", 0);
    if(nAnonymizeRenosCoinAmount > 999999) nAnonymizeRenosCoinAmount = 999999;
    if(nAnonymizeRenosCoinAmount < 2) nAnonymizeRenosCoinAmount = 2;
    ...
    if(fEnableInstantX){
        ...
        if(nInstantXDepth < 0) nAnonymizeRenosCoinAmount = 0;
    } else {
        nInstantXDepth = 0;
    }
    ...
    LogPrintf("Anonymize RenosCoin Amount %d\n", nAnonymizeRenosCoinAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1RenosCoin+1000 == (.1RenosCoin+100)*10
       10RenosCoin+10000 == (1RenosCoin+1000)*10
    */
